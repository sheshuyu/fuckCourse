...
def decode_course_point(html_text: str) -> Dict[str, Any]:
    ...
    for chapter_unit in soup.find_all("div", class_="chapter_unit"):
        # 提取章标题
        chapter_title_el = chapter_unit.select_one(".catalog_title.chapter_Thats_bnt .clicktitle span")
        chapter_title = chapter_title_el.get("title", "") or chapter_title_el.text.strip() if chapter_title_el else ""
        ...
        # 提取树形结构
        catalog_level = chapter_unit.select_one("div.catalog_level")
        sections = []
        if catalog_level:
            top_ul = catalog_level.find("ul")
            if top_ul:
                sections = _extract_section_tree(top_ul)

        course_point["chapters"].append({
            "title": chapter_title,
            "sections": sections,
        })
...
def _extract_points_from_chapter(chapter_unit) -> List[Dict[str, Any]]:
    """从章节单元中提取章节点信息（平铺，保持向后兼容）"""
    point_list = []
    raw_points = chapter_unit.find_all("li")

    for raw_point in raw_points:
        detail = _parse_chapter_item(raw_point)
        if detail:
            point_list.append(detail)

    return point_list


def _parse_chapter_item(li_element) -> Optional[Dict[str, Any]]:
    """从 li 元素提取单个章节项信息"""
    chapter_item = li_element.find("div", class_="chapter_item")
    if not chapter_item or "id" not in chapter_item.attrs:
        return None

    point_id_match = re.findall(r"^cur(\d{1,20})$", chapter_item.attrs["id"])
    if not point_id_match:
        return None

    point_id = point_id_match[0]
    clicktitle = chapter_item.select_one("a.clicktitle")
    if not clicktitle:
        return None
    point_title = clicktitle.text.replace("\n", "").strip()

    job_count = 1
    need_unlock = False
    if chapter_item.select_one("input.knowledgeJobCount"):
        job_count = chapter_item.select_one("input.knowledgeJobCount").attrs["value"]
    elif chapter_item.select_one("span.bntHoverTips") and "解锁" in chapter_item.select_one("span.bntHoverTips").text:
        need_unlock = True

    is_finished = False
    if chapter_item.select_one("span.bntHoverTips") and "已完成" in chapter_item.select_one("span.bntHoverTips").text:
        is_finished = True
...
def _extract_section_tree(ul_element) -> List[Dict[str, Any]]:
    """递归遍历 ul > li 结构，构建章节树"""
    sections = []
    for li in ul_element.find_all("li", recursive=False):
        detail = _parse_chapter_item(li)
        if not detail:
            continue
        children = []
        nested_ul = li.find("ul")
        if nested_ul:
            children = _extract_section_tree(nested_ul)
